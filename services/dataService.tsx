...
async function UpdateUserPointAndCoin(UserNewCoinPoint:IUserCoinPoint)
{
    let res = await fetch(`${link}/User/UpdateCoinsAndPoints`,{
        ...
        body: JSON.stringify({UserNewCoinPoint})
    });
    if(!res.ok)
    {
        const message = `An Error has Occured ${res.status}`
